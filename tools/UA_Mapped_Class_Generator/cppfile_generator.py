...
class cppfile_generator():
	...
	def __init__(self, serverList, generatedNamspaceFileName):
		self.ignoredNodes = []
	...
	def generateImplementationFile(self, implementation, objectNode, methodList, variableList, objectList):
		...
		if	self.isServerClass(classname):
			...
			implementation.write("#include \"ua_server.h\"\n")
			implementation.write("#include \"ua_types.h\"\n")
			implementation.write("#include \"ua_config_standard.h\"\n")
			implementation.write("#include \"ua_network_tcp.h\"\n")
			implementation.write("#include \"ua_log_stdout.h\"\n\n")
			implementation.write("// Model uses bootstrapping: Force it to use internal headers to find the necessary types\n")
			implementation.write("// by pretending we never amalgamated (installer puts the headers into the right place for us)\n")
			implementation.write("#ifndef UA_NO_AMALGAMATION\n")
			implementation.write("#define UA_NO_AMALGAMATION\n")
			implementation.write(INDENT + "#include MODEL_HEADER_NAME\n")
			implementation.write("#undef UA_NO_AMALGAMATION\n")
			implementation.write("#else\n")
			implementation.write(INDENT + "#include MODEL_HEADER_NAME\n")
